...
For an analytic solution, we address the reader to the @sec-analytic-solution, where we establish that
$$
...
##### The Lambert function $W_k(x)$

Otherwise than numerically, $\hat\lambda$ can be found using the Lambert function $W_0$.
...
##### Analytical solution to $ax=1-e^{-x}$ {#sec-analytic-solution}

Back to the estimation problem, the Lambert function $W_0(x)$ presented in the previous section will be useful after some transformation in original estimation equation.
...
Since $\bar{y}>1$, we have $-\bar{y}e^{-\bar{y}}<0$, which gives $W_0(-\bar{y}e^{-\bar{y}})<0$, so that $\laht\lambda$ is $\bar{y}$ minus a bias correction term that takes into account the fact that all zero observations were excluded before taking the average $\bar{y}$.
