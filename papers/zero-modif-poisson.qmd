...
expand_grid(
  ...
  ggtitle( 
    "Comparing regular, truncated, zero-deflated, and zero-inflated models" ,
    subtitle = 
      "Regular ($p=p_0$), truncated ($p=0$), " %$%
...
For an i.i.d. sample $\widetilde{X}=(X_1,\ldots,X_n)$, the log-likelihood function $\ell(\lambda|\widetilde{x})$, the score function $U(\lambda|\widetilde{x})$, observed information $\mathcal{J}(\lambda|\widetilde{x})$, and Fisher information $\mathcal{I}(\lambda)$ are
$$
\begin{align}
...
The log-likelihood function $\ell(\lambda|y)$, the score function $U(\lambda|y)$, observed information $\mathcal{J}(\lambda|y)$, and Fisher information $\mathcal{I}_Y(\lambda)$ of a single observation $Y \sim TruncPoi(\lambda)$ are
$$
\begin{align}
\ell(\lambda|y) 
  & = \log(P(Y=y)) 
...
\mathcal{J}(\lambda|y) 
  ...
  % \\
  % & \ell''(\lambda|y) \\
  % & = \frac{d}{d\lambda} (\frac{y}{\lambda} - \frac{1}{1-e^{-\lambda}}) \\
  % & = -\frac{y}{\lambda^2} - \frac{d}{d\lambda} \frac{1}{1-e^{-\lambda}} \\
  % & = -\frac{y}{\lambda^2} - \frac{d}{d\lambda} (1-e^{-\lambda})^{-1} \\
  % & = -\frac{y}{\lambda^2} - 
  %     (-1) (1-e^{-\lambda})^{-2} \frac{d}{d\lambda} (1-e^{-\lambda}) \\
  % & = -\frac{y}{\lambda^2} - 
  %     (-1) (1-e^{-\lambda})^{-2} \frac{d}{d\lambda} -e^{-\lambda} \\
  % & = -\frac{y}{\lambda^2} - 
  %     (1-e^{-\lambda})^{-2} \frac{d}{d\lambda} e^{-\lambda} \\
  % & = -\frac{y}{\lambda^2} - 
  %     (1-e^{-\lambda})^{-2} e^{-\lambda} (-1)\\
  % & = -\frac{y}{\lambda^2} + 
  %     (1-e^{-\lambda})^{-2} e^{-\lambda} \\
  % & = -\frac{y}{\lambda^2} + 
  ...
  %   \\
  % & (e^{\lambda/2}-e^{-\lambda/2})^2 \\
  % & = (e^{\lambda/2})^2 - 2 e^{\lambda/2} e^{-\lambda/2}+(e^{-\lambda/2})^2 \\
  % & = e^{\lambda} - 2 e^{\lambda/2-\lambda/2} + e^{-\lambda} \\
  % & = e^{\lambda} - 2 e^{0} + e^{-\lambda} \\
  % & = e^{\lambda} - 2 + e^{-\lambda} \\
  % & = e^{\lambda} + e^{-\lambda} - 2\\
  %   \\
    = \frac{y}{\lambda^2} - \frac{1}{e^{\lambda} + e^{-\lambda} - 2} \\
%
\mathcal{I}_X(\lambda) 
  & = \mathbb{E}_Y(\mathcal{J}(\lambda|Y)) % \\
  % & = \frac{\lambda}{\lambda^2} - \frac{1}{e^{\lambda} + e^{-\lambda} - 2} \\
    = \frac{1}{\lambda} - \frac{1}{e^{\lambda} + e^{-\lambda} - 2}
\end{align}
$$
unless a term constant with respect to $\lambda$.
